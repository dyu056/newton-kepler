...
Generates deterministic synthetic videos of a mass-spring oscillator with
two frequency bands (slow: red, fast: blue).  Each video is 129 frames at
20 fps, 128×128 RGB.

Usage:
    python build.py --config data.yaml --root data/spring_shortcuts_v4
    python build.py --config data.yaml --root data/spring_shortcuts_v4 --split train
    python build.py --config data.yaml --root data/spring_shortcuts_v4 --overwrite
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--split", choices=("all", "sanity", "train", "eval"), default="all"
    )
...
def main() -> None:
    ...
    (args.root / "build_summary.json").write_text(
