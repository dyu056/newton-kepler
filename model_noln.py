"""
LayerNorm-free copy of model_cv.py.

Standalone: this file duplicates model_cv.py in full so the architecture is explicit
and line-by-line comparable. model_cv.py is left untouched (read-only).

THE ONLY ARCHITECTURAL DIFFERENCE
---------------------------------
The three LayerNorm calls are gone. Concretely:
  * `Block.forward`  : `x + self.attn(self.ln_1(x))`  ->  `x + self.attn(x)`
                       `x + self.mlp(self.ln_2(x))`   ->  `x + self.mlp(x)`
  * `GPTCV.forward`  : `x = self.transformer.ln_f(x)` -> identity (kept as a module)

`ln_1`, `ln_2` and `ln_f` still EXIST as `nn.Identity()` submodules, under the same
attribute names. That is deliberate: probe.py:76 registers a forward hook on
`model.transformer.ln_f`, and probe.py:57-63 hooks `block.attn`, `block.mlp`,
`block.mlp.silu` and `block`. Keeping the names means every existing hook, and the
whole `observe.py` / `train.py` observation pipeline, works with zero changes.
`after_ln_f` then simply equals the raw residual stream.

Everything else is preserved verbatim: c_attn QKV projection and split, causal mask,
flash-attention path, `attention_alpha` position scaling, attention entropy capture
(both the no-grad stats and the differentiable penalty), MLP c_fc -> SiLU -> c_proj,
varcov penalties, both residual connections, wpe position embeddings,
input_embedding / output_head, the 0.02 init with the 0.02/sqrt(2*n_layer) special
init on c_proj, configure_optimizers, crop_block_size, estimate_mfu, generate.
Parameter count drops by exactly 2*n_embd per removed LayerNorm (768 for the
1-layer/128-dim config: 3 LayerNorms x weight+bias x 128).

WHY
---
With LayerNorm the origin is not a critical point of the gradient flow, so Lemma 3.1
of arXiv:2605.01199 (and the Prop 3.3 / 3.4 / Thm 3.5 cascade built on it) does not
apply here -- shrinking the init scale eps by 1e4 barely moves |dL/dW_out|
(8.20 -> 0.11, ratios 1.19 / 6.17 / 9.91 per 10x), because LN rescales the residual
stream back to O(1) regardless of eps. Without LayerNorm the gradients follow exact
integer powers of eps (eps^1 for W_out, eps^3 for W_fc, eps^7 for W_Q; ratios
10.00 / 999.07 / 1.00e7), so the origin is a genuine critical point.

Caveat measured in compare_noln_dip.py: removing LN also produces a long
O(eps) -> O(1) plateau. Over 120 GD steps at lr=1e-3 the loss moved only
1.1735 -> 1.1412 (versus 1.1077 -> 0.0642 with LN), so expect to need a larger
learning rate or many more steps before the later stages become visible.
"""
...
class Block(nn.Module):
    """Same as model_cv.Block, but ln_1 / ln_2 are Identity instead of LayerNorm."""

    def __init__(self, config, layer_name):
        super().__init__()
        self.ln_1 = nn.Identity()          # was: LayerNorm(config.n_embd, bias=config.bias)
        self.attn = CausalSelfAttention(config)
        self.ln_2 = nn.Identity()          # was: LayerNorm(config.n_embd, bias=config.bias)
        self.mlp = MLP(config, layer_name=layer_name)

    def forward(self, x):
        x = x + self.attn(self.ln_1(x))    # ln_1 is Identity -> x + self.attn(x)
        x = x + self.mlp(self.ln_2(x))     # ln_2 is Identity -> x + self.mlp(x)
        return x

@dataclass
class GPTConfigCVNoLN:
    block_size: int = 1024
    n_layer: int = 12
    n_head: int = 12
    n_embd: int = 768
...
class GPTCVNoLN(nn.Module):
    """
    GPT model for continuous variables, without LayerNorm.
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            ln_f = nn.Identity(),   # was: LayerNorm(config.n_embd, bias=config.bias)
        ))
    ...
    def forward(self, x, targets=None,compute_entropy=False):
        ...
        for block in self.transformer.h:
            x = block(x)
        x = self.transformer.ln_f(x)   # ln_f is Identity -> residual stream passes through
