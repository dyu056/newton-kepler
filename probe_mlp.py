"""Activation collection and linear probes for MLP-only model (model_mlp.py).

Differs from probe.py: hooks are compatible with MLPBlock (no attention),
and every intermediate activation inside the MLP is captured.
"""
...
from model_mlp import GPTConfigCV


# ── Hook setup ───────────────────────────────────────────────────────────

class _ActivationStore(dict):
    """Dict with a capture_enabled flag — hooks are no-ops when False."""
    capture_enabled: bool = False


def setup_activation_hooks(model, verbose=False):
    """Register hooks for every intermediate layer in the MLP-only model."""
    ...
    for block_idx in range(n_layer):
        ...
        # 1. MLP input (LN output via pre_hook on MLP)
        def make_mlp_input_hook(idx):
            def hook(module, inputs):
                save(f"block_{idx}_mlp_input", inputs)
            return hook
        hooks.append(block.mlp.register_forward_pre_hook(make_mlp_input_hook(block_idx)))

        # 2. After first Linear (c_fc output, pre-SiLU)
        def make_fc_output_hook(idx):
            def hook(module, inputs, output):
                save(f"block_{idx}_fc_output", output)
            return hook
        hooks.append(block.mlp.c_fc.register_forward_hook(make_fc_output_hook(block_idx)))

        # 3. After SiLU (hidden activation)
        def make_hidden_hook(idx):
            def hook(module, inputs, output):
                save(f"block_{idx}_mlp_hidden", output)
            return hook
        hooks.append(block.mlp.silu.register_forward_hook(make_hidden_hook(block_idx)))

        # 4. MLP output (c_proj + dropout, before residual merge)
        def make_mlp_output_hook(idx):
            def hook(module, inputs, output):
                save(f"block_{idx}_mlp_output", output)
            return hook
        hooks.append(block.mlp.register_forward_hook(make_mlp_output_hook(block_idx)))

        # 5. Block output (after residual merge)
        def make_block_output_hook(idx):
            def hook(module, inputs, output):
                save(f"block_{idx}_output", output)
            return hook
        hooks.append(block.register_forward_hook(make_block_output_hook(block_idx)))
    ...
    if verbose:
        print(f"Hooks registered for {n_layer} MLP block(s)")
        print(f"  Per block: mlp_input, fc_output, mlp_hidden, mlp_output, block_output")
        print(f"  Global: input_embed, after_pos_emb, after_ln_f")
...
def compute_gravitational_force(positions):
    """Compute 12 force/position targets from (B, T, 2) coordinates."""
    if torch.is_tensor(positions):
        x, y = positions[..., 0], positions[..., 1]
        r = torch.sqrt(x.square() + y.square())
        r3 = r.pow(3).clamp_min(1e-10)
        fx, fy = -x / r3, -y / r3
        mag = torch.sqrt(fx.square() + fy.square())
        safe_mag = mag.clamp_min(1e-10)
        safe_r = r.clamp_min(1e-10)
        safe_r2 = r.square().clamp_min(1e-10)
        safe_r3 = r.pow(3).clamp_min(1e-10)
        return {
            "Fx": fx.reshape(-1), "Fy": fy.reshape(-1),
            "F_magnitude": mag.reshape(-1),
            "F_direction_x": (fx / safe_mag).reshape(-1),
            "F_direction_y": (fy / safe_mag).reshape(-1),
            "r": r.reshape(-1),
    ...
    fx, fy = -positions[:, :, 0] / r3, -positions[:, :, 1] / r3
    mag = np.sqrt(fx ** 2 + fy ** 2)
    safe_mag = np.where(mag < 1e-10, 1e-10, mag)
    return {
        "Fx": fx.flatten(), "Fy": fy.flatten(),
        "F_magnitude": mag.flatten(),
        "F_direction_x": (fx / safe_mag).flatten(),
        "F_direction_y": (fy / safe_mag).flatten(),
        "r": r.flatten(),
        ...
        "x": positions[:, :, 0].flatten(), "y": positions[:, :, 1].flatten(),
    }
...
# ── Torch-native probe fitting (stays on GPU) ────────────────────────────
...
def _fit_single_target_probe(train_x, train_y, eval_x):
    ...
    train_pred = ((train_x - x_mean) @ weight + y_mean).reshape(-1)
    eval_pred = ((eval_x - x_mean) @ weight + y_mean).reshape(-1)
...
def run_linear_probes(train_activation_dict, train_gravitational_force,
                      ...
                      verbose=False):
    """Fit every force target as an independent linear probe, GPU-native."""
    ...
    for layer_name, train_activations in train_activation_dict.items():
        ...
        eval_bs, eval_ts, _ = eval_activations.shape
        ...
        for probe_name in probe_targets:
            ...
            eval_force = _reshape_token_target_torch(
                eval_gravitational_force[probe_name], eval_bs, eval_ts,
...
def run_geometry_probes(train_activation_dict, train_orbital_params,
                        train_trajectory_ids,
                        eval_activation_dict, eval_orbital_params,
                        eval_trajectory_ids):
    """Fit geometry probes on train sequences, score on held-out eval."""
    base_targets = [
    ...
    results = {}
    for layer_name, train_act in train_activation_dict.items():
        eval_act = eval_activation_dict[layer_name]
        train_all = train_act.reshape(-1, train_act.shape[-1]).cpu().numpy()
        eval_all = eval_act.reshape(-1, eval_act.shape[-1]).cpu().numpy()
        train_last = train_act[:, -1, :].cpu().numpy()
        eval_last = eval_act[:, -1, :].cpu().numpy()
        layer_results = {}
        for name in probe_targets:
            train_tgt_all = np.repeat(train_values[name], train_act.shape[1])
            eval_tgt_all = np.repeat(eval_values[name], eval_act.shape[1])
            p_all = LinearRegression().fit(train_all, train_tgt_all)
            p_last = LinearRegression().fit(train_last, train_values[name])
            train_r2_all = safe_r2_score(train_tgt_all, p_all.predict(train_all))
            eval_r2_all = safe_r2_score(eval_tgt_all, p_all.predict(eval_all))
            train_r2_last = safe_r2_score(train_values[name], p_last.predict(train_last))
            eval_r2_last = safe_r2_score(eval_values[name], p_last.predict(eval_last))
            layer_results[name] = {
