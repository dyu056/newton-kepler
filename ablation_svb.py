...
def svb_apply(model, epsilon=0.2):
    if epsilon <= 0.0:
        return
...
def train_model(model, train_inputs, train_targets, test_inputs, test_targets, train_trajectories, test_trajectories,
    ...
    for i in range(n_steps):
        ...
        # SVB: Singular Value Bounding — clamp singular values of Linear layers
        if svb_epsilon > 0.0 and (i + 1) % svb_freq == 0:
            svb_apply(model, svb_epsilon)
...
def train_one_model(block_size=100, noise_scale=0.1, lr=1e-3, n_layer=2, n_embd=16, num_trajectories=10000,
    ...
    training_results = train_model(
        ...
        svb_epsilon=svb_epsilon, svb_freq=svb_freq
    )
