...
# ── Variance-Covariance + Attention Entropy Regularization ──

def compute_varcov_penalty(representation, target_std=1.0, eps=1e-4):
    """
    representation: [batch, tokens, hidden] or [batch*tokens, hidden]

    Returns:
        variance_loss, covariance_loss, mean_std
    """
    ...
    if z.size(0) > 1 and z.size(1) > 1:
        ...
        off_diag = ~torch.eye(covariance.size(0), dtype=torch.bool, device=covariance.device)
        covariance_loss = covariance[off_diag].square().mean()
    else:
...
def compute_attention_entropy(attn_weights, eps=1e-12):
    """
    attn_weights: [batch, heads, seq, seq]  post-softmax attention weights

    Returns mean entropy over all heads and query positions.
    ...
    entropy_per_pos = -(p * torch.log(p)).sum(dim=-1)  # [B, H, seq]
    return entropy_per_pos.mean()


class RegHooks:
    """Register hooks to capture MLP representations and attention weights for regularization."""

    def __init__(self, model):
        self.model = model
        self.variance_losses = []
        self.covariance_losses = []
        self.attn_weights = []
        self.hooks = []
        self.mean_stds = []

        for block in model.transformer.h:
            # Hook MLP projection output (before dropout)
            h1 = block.mlp.c_proj.register_forward_hook(self._make_mlp_hook())
            self.hooks.append(h1)

            # Hook attention weights (if block has attention)
            if hasattr(block, 'attn'):
                h2 = block.attn.register_forward_hook(self._make_attn_hook())
                self.hooks.append(h2)

    def _make_mlp_hook(self):
        def hook(module, input, output):
            var_loss, cov_loss, mean_std = compute_varcov_penalty(output)
            self.variance_losses.append(var_loss)
            self.covariance_losses.append(cov_loss)
            self.mean_stds.append(mean_std)
        return hook

    def _make_attn_hook(self):
        def hook(module, input, output):
            pass  # attention weights captured separately during forward
        return hook

    def total_variance(self):
        if not self.variance_losses:
            return torch.tensor(0.0)
        return torch.stack([v for v in self.variance_losses if v is not None]).sum() if self.variance_losses else torch.tensor(0.0)

    def total_covariance(self):
        if not self.covariance_losses:
            return torch.tensor(0.0)
        return torch.stack([c for c in self.covariance_losses if c is not None]).sum() if self.covariance_losses else torch.tensor(0.0)

    def reset(self):
        self.variance_losses.clear()
        self.covariance_losses.clear()
        self.attn_weights.clear()
        self.mean_stds.clear()
...
def train_model(model, train_inputs, train_targets, test_inputs, test_targets, train_trajectories, test_trajectories,
                 ...
                 reg_hooks=None, variance_reg_weight=0.0, covariance_reg_weight=0.0):
    """
    ...
    for i in range(n_steps):
        ...
        predictions, _ = model.forward(inputs_noised, None)
        mse_loss = compute_loss_with_mask(predictions, batch_targets, loss_mask=loss_mask)

        # Compute regularization terms from captured hooks
        variance_source = reg_hooks.total_variance()
        covariance_source = reg_hooks.total_covariance()

        total_loss = mse_loss

        if variance_reg_weight > 0.0:
            total_loss = total_loss + variance_reg_weight * variance_source

        if covariance_reg_weight > 0.0:
            total_loss = total_loss + covariance_reg_weight * covariance_source
        ...
        if i % 100 == 0:
            ...
            print(f"Step {i}, Train Loss: {mse_loss.item():.6f}, Test Loss: {test_loss.item():.6f}, "
...
def train_one_model(block_size=100, noise_scale=0.1, lr=1e-3, n_layer=2, n_embd=16, num_trajectories=10000,
                    ...
                    variance_reg_weight=0.0, covariance_reg_weight=0.0):
    """
    ...
    # Setup regularization hooks (variance-covariance on MLP representations)
    reg_hooks = RegHooks(model)
    ...
    training_results = train_model(
        ...
        reg_hooks=reg_hooks, variance_reg_weight=variance_reg_weight,
...
def main():
    ...
    sweep_parameters(block_size_list, num_trajectories_list, noise_scale_list, loss_mask_list,
                     ...
                     variance_reg_weight=0.05, covariance_reg_weight=0.05)
